...
# 全局变量
ROAD_WIDTH = 1.5 #7.0  # 道路的最大宽度
WIDTH_GAP = 0.4 #1.0  # 道路的横向采样间隔
MIN_T = 8.0 #4.0  # 最短时间开销
MAX_T = 10.0 #5.0  # 最长时间开销
T_GAP = 0.1  # 时间采样间隔
ESP = 1e-6  # 精度要求
MAX_VELOCITY = 0.5 #50.0 / 3.6  # 最大速度
TARGET_VELOCITY = 0.3 #30.0 / 3.6  # 期望速度
VELOCITY_GAP = 0.05 #5.0 / 3.6  # 速度采样间隔
VELOCITY_SAMPLE_N = 1  # 速度采样个数
MAX_KAPPA = 0.8  # 最大曲率
MAX_ACCELERATION = 0.05 #2.0  # 最大加速度
ROBOT_SIZE = 0.5 #2.0  # 机器人的半径
AREA = 3.0  # 可视化窗口大小
...
class QuinticPolynomial:
    ...
    def __coefficientGeneration(self):
        ...
        A = np.array([[self.T_ ** 3, self.T_ ** 4, self.T_ ** 5],
                      [3. * self.T_ ** 2, 4. * self.T_ ** 3, 5. * self.T_ ** 4], 
                      [6. * self.T_, 12. * self.T_ ** 2, 20. * self.T_ ** 3]])
        b = np.array([self.params_[3] - a0 - a1 * self.T_ - a2 * self.T_ ** 2, 
                      self.params_[4] - a1 - 2. * a2 * self.T_, 
                      self.params_[5] - 2. * a2]).T
...
class QuarticPolynomial:
    ...
    def __coefficientGeneration(self):
        ...
        a1 = self.vs_
        a2 = self.acs_ * 0.5
        A = np.array([[3. * self.T_ ** 2, 4. * self.T_ ** 3], 
                      [6. * self.T_, 12. * self.T_ ** 2]])
...
# frenet坐标架下的规划
def frenetOptimalTrajectoryPlanning(current_s, current_s_derivative, current_s_2derivative, 
                                    current_d, current_d_derivative, curent_d_2derivative):
    # 生成轨迹组
    print( len(np.arange(MIN_T, MAX_T + ESP, T_GAP)) )
    print( len(np.arange(-ROAD_WIDTH, ROAD_WIDTH, WIDTH_GAP)) )
    print( len(np.arange(TARGET_VELOCITY - VELOCITY_GAP * VELOCITY_SAMPLE_N, 
                                           TARGET_VELOCITY + VELOCITY_GAP * VELOCITY_SAMPLE_N + ESP, VELOCITY_GAP)) )
    ...
    for df_offset in np.arange(-ROAD_WIDTH, ROAD_WIDTH, WIDTH_GAP):
        for T in np.arange(MIN_T, MAX_T + ESP, T_GAP):
            ...
            for sf_derivative in np.arange(TARGET_VELOCITY - VELOCITY_GAP * VELOCITY_SAMPLE_N, 
                                           TARGET_VELOCITY + VELOCITY_GAP * VELOCITY_SAMPLE_N + ESP, VELOCITY_GAP):
                ...
                # print("cost_", frenet_path_copy.cost_)
...
def main():
    ...
    wx = [0.0,  1.5,  1.3,  1.6, 1.4]
    wy = [0.0, 1.5, 3.5, 5.5, 7.5]
    ...
    current_s_derivative = 0.2 #10.0 / 3.6  # frenet坐标下的s轴对时间的一阶导数
    current_s_2derivative = 0.0  # frenet坐标下的s轴对时间的二阶导数
    current_d = 0.0  # frenet坐标下的d轴
    ...
    while True:
        ...
        frenet_path_set = frenetOptimalTrajectoryPlanning(current_s, current_s_derivative, current_s_2derivative, 
                                                          current_d, current_d_derivative, curent_d_2derivative)
        print("frenet_path_set ", len(frenet_path_set) )
        # 将frenet坐标系转换到世界坐标系下
        ...
        print("checked_path_set ", len(checked_path_set) )
        # print(len(checked_path_set))
        # 从其中选取一条代价最低的作为最终路径
        # min_cost = float("inf")
        final_path = min(checked_path_set, key=lambda p: p.cost_)

        print('planning took %fs!' % (time.time() - start_time))
        # 新的下标
